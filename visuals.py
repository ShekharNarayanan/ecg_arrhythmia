...
def annotate_counts(ax, labels, keep_labels):
    for i, beat in enumerate(keep_labels):
        ...
        ax.annotate(f"n={count}", xy=(i, 0), xycoords=('data', 'axes fraction'),
                    ha='center', fontsize=8, color='gray')

# --------------------------------------------- features -----------------------------------------------------------------------------------------------------
# get waveforms for participant
raw_signal, fs, channels, r_peaks, beat_labels = file_utils.load_raw_participant_data(raw_data_path=raw_data_path, participant_id=participant)
...
# processed waveforms
processed_sig = preprocess.get_preprocessed_signal(raw_sig=raw_signal[:,chan],fs=fs,bandpass_window=bandpass_window)

# wfms
wfms = extract_features.extract_waveforms(ecg_signal=processed_sig,
                                        fs=fs,
                                        r_peaks=r_peaks,
                                        window_start_ms=wave_extraction_window[0],
                                        window_end_ms=wave_extraction_window[1])
rr_features_dict = extract_features.extract_all_rr_features(r_peaks=r_peaks,local_rr_mean_beat_window=local_rr_mean_beat_window,rr_irregularity_window=rr_irregularity_window)
...
for beat in keep_labels:
    m = labels == beat
    ax.scatter(rr_features["pre_rr"][m] / fs * 1000, rr_features["post_rr"][m] / fs * 1000,
               label=f"{beat}:  count - {np.sum(m)}", color=colors[beat], alpha=0.4, s=10)
...
ax.plot(time_axis, rr_features["local_rr_mean"] / fs * 1000, color='lightgray', zorder=1)
for beat in keep_labels:
    m = labels == beat
    ax.scatter(time_axis[m], rr_features["local_rr_mean"][m] / fs * 1000,
               label=beat, color=colors[beat], s=15, zorder=2, alpha=0.7)
...
feature_names = ["pre_rr","post_rr","delta_rr","rr_ratio","local_rr_mean","deviation_rr","rr_irreg","q_r_int","r_s_int","qrs_width"]
